...
def add_item(entry: Entry, listbox: Listbox):
    ...
    with open('D:/internship2024/tasks.txt', 'a') as tasks_list_file:
...
def delete_item(listbox: Listbox):
    ...
    with open('D:/internship2024/tasks.txt', 'r+') as tasks_list_file:
...
root = Tk()
root.title('Amrutha J')
root.geometry('300x400')
root.resizable(0, 0)
root.config(bg="Green")

# Heading Label
Label(root, text='Amrutha to do list', bg='White', font=("Comic Sans MS", 15), wraplength=300).place(
    x=35, y=0)
...
with open('D:/internship2024/tasks.txt', 'r+') as tasks_list:
